...
{% hint style="info" %}
If you already have a `products` Flow, skip to [Create a Flow Field](./#3-create-a-flow-field). You'll need the ID of the products Flow to continue.
{% endhint %}
...
curl -X POST https://api.moltin.com/v2/products \
    ...
    -d $'{
       "data": {
         "type": "product",
           "name" : "Very scary horror",
           "slug": "very_scary_horror",
           "sku": "SH01-3",
           "manage_stock": false,
           ...
           "price": [{
             "amount": 1299, "currency": "USD", "includes_tax": true
            }],
...
{% tab title="cURL Request" %}
```bash
curl -X PUT https://api.moltin.com/v2/products \
     -H "Authorization: XXXX" \
     -H "Content-Type: application/json" \
     -d $'{
        "data": {
